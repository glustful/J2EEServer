...
import com.dunbian.jujiabao.wcpay.utils.TenpayUtil;
 ...
 * 
 */
public class PrOrder {// Key

	/**
	 * 获取最后支付所用的pakage；
	 */
	public static SortedMap<String, String> createPrOrder(String productMsg,String prIp,String totalFee,String orderId,String openId,String notifyurl) {
		
		//微信支付jsApi
		WxPayDto tpWxPay = new WxPayDto();
		tpWxPay.setOpenId(openId);
		tpWxPay.setBody(productMsg);
		tpWxPay.setOrderId(orderId);
		tpWxPay.setSpbillCreateIp(prIp);
...
//		getCodeurl(tpWxPay1);
	...
	public static String getCodeurl(WxPayDto tpWxPayDto){
		...
		String xml = "<xml>" + "<appid>" + Config.appid + "</appid>" + "<mch_id>"
	...
	public static SortedMap<String, String>  getPackage(WxPayDto tpWxPayDto) {
		...
		String attach = "无";// 附加数据 原样返回
		String totalFee = getMoney(tpWxPayDto.getTotalFee());// 总金额以分为单位，不带小数点
		String spbill_create_ip = tpWxPayDto.getSpbillCreateIp();// 订单生成的机器 IP
		String notify_url = tpWxPayDto.getNotifyUrl();// 这里notify_url是 支付完成后微信发给该链接信息，可以判断会员是否支付成功，改变订单状态等。
		String trade_type = "JSAPI";

		// ---必须参数
		String mch_id = Config.partner;// 商户号
		...
		finalpackage.put("appId", Config.appid);
		...
		String finaPackage = "\"appId\":\"" + Config.appid + "\",\"timeStamp\":\"" + timestamp
