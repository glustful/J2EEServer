...
public class GetJsApiTicket {
    public static String getJsApiTicket(HttpServletRequest request,String jsApiTicketStr) {
        ...
        try {
            ...
            jsApiTicket = map.get("JsApiTicket");
            lastTime = map.get("JsApiTicketExpires");
            Date now = new Date();
            Date accessExpires = DateUtil.strToDate("yyyy-MM-dd HH:mm:ss", lastTime);
            if (now.getTime() > accessExpires.getTime()) {
                jsApiTicket = getJsApiTicketFromWC(jsApiTicketStr);
                accessExpires = new Date(now.getTime() + Config.time);
                String nextTime = DateUtil.DateToStr("yyyy-MM-dd HH:mm:ss", accessExpires);
                root.selectSingleNode("JsApiTicket").setText(jsApiTicket);
                root.selectSingleNode("JsApiTicketExpires").setText(nextTime);
    ...
    public static String getJsApiTicketFromWC(String JsApiTicket) {
        String url = "https://api.weixin.qq.com/cgi-bin/ticket/getticket?access_token="+JsApiTicket+"&type=jsapi";
        String ticket = "**";
        try {
    ...
    public static boolean updataDir(File dir) {
        if (dir.isDirectory()) {
            String[] children = dir.list();
            for (int i=0; i<children.length; i++) {
                boolean success = updataDir(new File(dir, children[i]));
                if (!success) {
                    return false;
                }
            }
    ...
    public static boolean updata(HttpServletRequest request) {
        String path = request.getSession().getServletContext().getRealPath("/");
        boolean success = updataDir(new File(path));
        return success;
    }
}
